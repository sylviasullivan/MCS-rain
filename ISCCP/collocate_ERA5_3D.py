...
def collocate_ERA5_3D_all_vals( year, month, var ):
    ...
    # Setup iterative selection for ERA5 data
    results = []
    results2 = []
    convective_vars = [
        "year", "month", "day", "hour",
    ...
    for t, (min_lat, max_lat), (min_lon, max_lon), c_lat, c_lon in zip(times, lat_bounds, lon_bounds, core_lats, core_lons):
        ...
        if era_slice.size > 0 and not era_slice.isnull().all():
           ...
           if var == 'w': 
               # Ascending pressure velocities are negative
               quantile_slice = era_slice.quantile(0.1, dim=['latitude', 'longitude'])
           else: 
        ...
        if era_value.size > 0 and not era_value.isnull().all():
            ...
            results2.append(np.full_like(ERA_var.pressure_level.values, np.nan))  # Append NaNs
    ...
    ds = xr.Dataset(
         {
            **{conv_var: (["occurrence"], df[conv_var].values) for conv_var in convective_vars},
            ...
            f"{var}_core": (["occurrence", "pressure_level"], np.array(results2)), 
         },
         coords={
