#!/bin/sh
#SBATCH --job-name=precip5_iterate
...
EOL
    ...
    sbatch --wait --output=/dev/null /groups/sylvia/JAS-MCS-rain/ISCCP/submit_PRECIP5request.sh
    # Wait until this job is done

    #python collocate_ERA5_2D_all_vals.py ${year} ${month} 'cape'
    #python collocate_ERA5_2D_core_vals.py ${year} ${month} 'cape'
  done
done
