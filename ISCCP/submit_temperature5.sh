...
EOL
    ...
    sbatch --wait --output=/dev/null /groups/sylvia/JAS-MCS-rain/ISCCP/submit_T5request.sh
    # Wait until this job is done

    python collocate_ERA5_vectorized.py ${year} ${month} 'temperature'
  done
done
