...
c.retrieve(
 ...
 '/xdisk/sylvia/ERA5_output/ERA5_pacc_tropical.nc')
EOL

    sbatch --wait --output=/dev/null /groups/sylvia/JAS-MCS-rain/ISCCP/submit_PACC5request.sh
    # Wait until this job is done

    python collocate_ERA5_2D_all_vals.py ${year} ${month} 'pacc'
    python collocate_ERA5_2D_core_vals.py ${year} ${month} 'pacc'
  done
done
